/**
 * Developer tool: deterministic throughput benchmark for the DSP engine.
 *
 * ADR-005 deferred the spec's broader CPU optimisations because this
 * container cannot produce a trustworthy *absolute* CPU number (shared,
 * virtualised, no fixed clock). That is still true, and this tool does not
 * change it: treat the absolute figures below as meaningless.
 *
 * What it is good for is *relative* comparison - run it before a change and
 * after, on the same machine in the same session, and the ratio is
 * informative even here. That is exactly what was missing when ADR-005 had
 * to defer optimisation work for lack of any measurement at all.
 *
 * Adapted from the parallel agent branch (agent/world-class-dsp-phase3, see
 * ADR-011); the methodology - warm-up before timing, several sample rate /
 * block size / instance-count combinations, results normalised both per
 * sample and as a percentage of realtime - was sound and is kept as-is.
 *
 * Release builds only. Not part of the shipped plugin; build with
