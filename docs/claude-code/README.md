...
| Deterministic scripts | `scripts/*.sh` | Explicitly run | Repository state, static audit, build/test/render/benchmark |
...
1. `/ui-ux-audit` — define the target musician, first useful result, information architecture, dependencies, responsive sizes, and evidence plan.
2. Trace parameters, UI state, automation, presets, warnings, and the visualiser before changing layout.
3. Implement the smallest coherent user-flow improvement.
4. Generate the required screenshot matrix at 820x650, 960x700, and 1440x900.
5. Delegate a read-only review to `ui-ux-auditor`.
6. Test keyboard traversal, resizing, host scaling, tooltips, warnings, preset revert, and DAW shortcut coexistence.
7. Distinguish implemented, screenshot-verified, keyboard-tested, host-validated, and observed-user validation.
